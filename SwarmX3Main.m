...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX2.mat;               % Load save A
load LeaderX2.mat;                            % Load save B
load ObstacleX2.mat;                          % Load save C
load TimeWindowX2.mat;                        % Load save D 
load IntersectionPolyshapeX2.mat              % Load save E
load DetectionWindowX2.mat;                   % Load save F
load IntersectionPolyshapeX2P.mat             % Load save E (Puddle)
load ObstacleX2P.mat;                         % Load save C (Puddle)
load DetectionWindowX2P.mat;                  % Load save F (Puddle)
load TotalResultsX2.mat;                      % Load save G
...
v=VideoWriter('X3_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX3.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_3 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X2) - Follower(X3) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX3A=figure('Name','Leader: Agent X2 | Follower: Agent X3');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X2 | Follower: Agent X3');
...
close(v);
savefig(FigureX3A,'FigureX3A.fig');
...
save LeaderX3.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX3.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX3P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX3.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX3.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX3P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX3.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX3P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX3.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX3B=figure('Name','Leader:  X2 | Follower: Agent X3 | Obstacle realtime progression - 2D');title('Leader: Agent X2 | Follower: Agent X3 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X3_RealTimeObstacle.avi');
   open(v);
   for i=1:m
