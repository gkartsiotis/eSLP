...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX20.mat;              % Load save A
load LeaderX20.mat;                           % Load save B
load ObstacleX20.mat;                         % Load save C
load TimeWindowX20.mat;                       % Load save D 
load IntersectionPolyshapeX20.mat             % Load save E
load DetectionWindowX20.mat;                  % Load save F
load IntersectionPolyshapeX20P.mat             % Load save E (Puddle)
load ObstacleX20P.mat;                        % Load save C (Puddle)
load DetectionWindowX20P.mat;                 % Load save F (Puddle)
load TotalResultsX20.mat;                     % Load save G
...
v=VideoWriter('X21_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX21.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_21 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X19) - Follower(X20) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX21A=figure('Name','Leader: Agent X20 | Follower: Agent X21');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X20 | Follower: Agent X21');
...
close(v);
savefig(FigureX21A,'FigureX20A.fig');
...
save LeaderX21.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX21.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX21P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX21.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX21.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX21P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX20.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX21P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX21.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX21B=figure('Name','Leader:  X20 | Follower: Agent X21 | Obstacle realtime progression - 2D');title('Leader: Agent X20 | Follower: Agent X21 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X21_RealTimeObstacle.avi');
   open(v);
   for i=1:m
