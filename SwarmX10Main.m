...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX9.mat;               % Load save A
load LeaderX9.mat;                            % Load save B
load ObstacleX9.mat;                          % Load save C
load TimeWindowX9.mat;                        % Load save D 
load IntersectionPolyshapeX9.mat              % Load save E
load DetectionWindowX9.mat;                   % Load save F
load IntersectionPolyshapeX9P.mat             % Load save E (Puddle)
load ObstacleX9P.mat;                         % Load save C (Puddle)
load DetectionWindowX9P.mat;                  % Load save F (Puddle)
load TotalResultsX9.mat;                      % Load save G
...
v=VideoWriter('X10_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX10.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_10 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X9) - Follower(X110) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX10A=figure('Name','Leader: Agent X9 | Follower: Agent X10');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X9 | Follower: Agent X10');
...
close(v);
savefig(FigureX10A,'FigureX10A.fig');
...
save LeaderX10.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX10.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX10P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX10.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX10.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX10P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX10.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX10P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX10.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX10B=figure('Name','Leader:  X9 | Follower: Agent X10 | Obstacle realtime progression - 2D');title('Leader: Agent X9 | Follower: Agent X10 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X10_RealTimeObstacle.avi');
   open(v);
   for i=1:m
