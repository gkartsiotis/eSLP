...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX11.mat;              % Load save A
load LeaderX11.mat;                           % Load save B
load ObstacleX11.mat;                         % Load save C
load TimeWindowX11.mat;                       % Load save D 
load IntersectionPolyshapeX11.mat             % Load save E
load DetectionWindowX11.mat;                  % Load save F
load IntersectionPolyshapeX11P.mat             % Load save E (Puddle)
load ObstacleX11P.mat;                        % Load save C (Puddle)
load DetectionWindowX11P.mat;                 % Load save F (Puddle)
load TotalResultsX11.mat;                     % Load save G
...
v=VideoWriter('X12_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX12.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_12 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X11) - Follower(X12) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX12A=figure('Name','Leader: Agent X11 | Follower: Agent X12');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X11 | Follower: Agent X12');
...
close(v);
savefig(FigureX12A,'FigureX12A.fig');
...
save LeaderX12.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX12.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX12P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX12.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX12.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX12P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX12.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX12P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX12.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX12B=figure('Name','Leader:  X11 | Follower: Agent X12 | Obstacle realtime progression - 2D');title('Leader: Agent X11 | Follower: Agent X12 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X12_RealTimeObstacle.avi');
   open(v);
   for i=1:m
