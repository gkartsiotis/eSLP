...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX7.mat;               % Load save A
load LeaderX7.mat;                            % Load save B
load ObstacleX7.mat;                          % Load save C
load TimeWindowX7.mat;                        % Load save D 
load IntersectionPolyshapeX7.mat              % Load save E
load DetectionWindowX7.mat;                   % Load save F
load IntersectionPolyshapeX7P.mat             % Load save E (Puddle)
load ObstacleX7P.mat;                         % Load save C (Puddle)
load DetectionWindowX7P.mat;                  % Load save F (Puddle)
load TotalResultsX7.mat;                      % Load save G
...
v=VideoWriter('X7_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX8.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_8 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X7) - Follower(X8) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX8A=figure('Name','Leader: Agent X7 | Follower: Agent X8');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X7 | Follower: Agent X8');
...
close(v);
savefig(FigureX8A,'FigureX8A.fig');
...
save LeaderX8.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX8.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX8P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX8.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX8.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX8P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX8.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX8P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX8.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX8B=figure('Name','Leader:  X7 | Follower: Agent X8 | Obstacle realtime progression - 2D');title('Leader: Agent X7 | Follower: Agent X8 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X8_RealTimeObstacle.avi');
   open(v);
   for i=1:m
