...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX16.mat;              % Load save A
load LeaderX16.mat;                           % Load save B
load ObstacleX16.mat;                         % Load save C
load TimeWindowX16.mat;                       % Load save D 
load IntersectionPolyshapeX16.mat             % Load save E
load DetectionWindowX16.mat;                  % Load save F
load IntersectionPolyshapeX16P.mat             % Load save E (Puddle)
load ObstacleX16P.mat;                        % Load save C (Puddle)
load DetectionWindowX16P.mat;                 % Load save F (Puddle)
load TotalResultsX16.mat;                     % Load save G
...
v=VideoWriter('X17_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX17.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_17 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X16) - Follower(X17) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX17A=figure('Name','Leader: Agent X16 | Follower: Agent X17');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X16 | Follower: Agent X17');
...
close(v);
savefig(FigureX17A,'FigureX17A.fig');
...
save LeaderX17.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX17.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX17P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX17.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX17.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX17P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX17.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX17P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX17.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX17B=figure('Name','Leader:  X16 | Follower: Agent X17 | Obstacle realtime progression - 2D');title('Leader: Agent X16 | Follower: Agent X17 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X17_RealTimeObstacle.avi');
   open(v);
   for i=1:m
