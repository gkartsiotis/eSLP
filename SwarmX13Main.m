...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX12.mat;              % Load save A
load LeaderX12.mat;                           % Load save B
load ObstacleX12.mat;                         % Load save C
load TimeWindowX12.mat;                       % Load save D 
load IntersectionPolyshapeX12.mat             % Load save E
load DetectionWindowX12.mat;                  % Load save F
load IntersectionPolyshapeX12P.mat             % Load save E (Puddle)
load ObstacleX12P.mat;                        % Load save C (Puddle)
load DetectionWindowX12P.mat;                 % Load save F (Puddle)
load TotalResultsX12.mat;                     % Load save G
...
v=VideoWriter('X13_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX13.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_13 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X12) - Follower(X13) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX13A=figure('Name','Leader: Agent X12 | Follower: Agent X13');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X12 | Follower: Agent X13');
...
close(v);
savefig(FigureX13A,'FigureX13A.fig');
...
save LeaderX13.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX13.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX13P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX13.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX13.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX13P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX13.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX13P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX13.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX13B=figure('Name','Leader:  X12 | Follower: Agent X13 | Obstacle realtime progression - 2D');title('Leader: Agent X12 | Follower: Agent X13 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X13_RealTimeObstacle.avi');
   open(v);
   for i=1:m
