...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX14.mat;              % Load save A
load LeaderX14.mat;                           % Load save B
load ObstacleX14.mat;                         % Load save C
load TimeWindowX14.mat;                       % Load save D 
load IntersectionPolyshapeX14.mat             % Load save E
load DetectionWindowX14.mat;                  % Load save F
load IntersectionPolyshapeX14P.mat             % Load save E (Puddle)
load ObstacleX14P.mat;                        % Load save C (Puddle)
load DetectionWindowX14P.mat;                 % Load save F (Puddle)
load TotalResultsX14.mat;                     % Load save G
...
v=VideoWriter('X15_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX15.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_15 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X14) - Follower(X15) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX15A=figure('Name','Leader: Agent X14 | Follower: Agent X15');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X14 | Follower: Agent X15');
...
close(v);
savefig(FigureX15A,'FigureX15A.fig');
...
save LeaderX15.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX15.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX15P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX15.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX15.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX15P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX15.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX15P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX15.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX15B=figure('Name','Leader:  X14 | Follower: Agent X15 | Obstacle realtime progression - 2D');title('Leader: Agent X14 | Follower: Agent X15 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X15_RealTimeObstacle.avi');
   open(v);
   for i=1:m
