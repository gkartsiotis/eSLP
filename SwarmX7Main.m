...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX6.mat;               % Load save A
load LeaderX6.mat;                            % Load save B
load ObstacleX6.mat;                          % Load save C
load TimeWindowX6.mat;                        % Load save D 
load IntersectionPolyshapeX6.mat              % Load save E
load DetectionWindowX6.mat;                   % Load save F
load IntersectionPolyshapeX6P.mat             % Load save E (Puddle)
load ObstacleX6P.mat;                         % Load save C (Puddle)
load DetectionWindowX6P.mat;                  % Load save F (Puddle)
load TotalResultsX6.mat;                      % Load save G
...
save AlgorithmParametersX7.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_7 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X6) - Follower(X7) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX7A=figure('Name','Leader: Agent X6 | Follower: Agent X7');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X6 | Follower: Agent X7');
...
close(v);
savefig(FigureX7A,'FigureX7A.fig');
...
save LeaderX7.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX7.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX7P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX7.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX7.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX7P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX7.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX7P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX7.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX7B=figure('Name','Leader:  X6 | Follower: Agent X7 | Obstacle realtime progression - 2D');title('Leader: Agent X6 | Follower: Agent X7 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X7_RealTimeObstacle.avi');
   open(v);
   for i=1:m
