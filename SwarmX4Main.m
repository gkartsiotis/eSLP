...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX3.mat;               % Load save A
load LeaderX3.mat;                            % Load save B
load ObstacleX3.mat;                          % Load save C
load TimeWindowX3.mat;                        % Load save D 
load IntersectionPolyshapeX3.mat              % Load save E
load DetectionWindowX3.mat;                   % Load save F
load IntersectionPolyshapeX3P.mat             % Load save E (Puddle)
load ObstacleX3P.mat;                         % Load save C (Puddle)
load DetectionWindowX3P.mat;                  % Load save F (Puddle)
load TotalResultsX3.mat;                      % Load save G
...
v=VideoWriter('X4_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX4.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_4 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X3) - Follower(X4) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX4A=figure('Name','Leader: Agent X3 | Follower: Agent X4');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X3 | Follower: Agent X4');
...
close(v);
savefig(FigureX4A,'FigureX4A.fig');
...
save LeaderX4.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX4.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX4P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX4.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX4.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX4P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX4.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX4P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX4.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX4B=figure('Name','Leader:  X3 | Follower: Agent X4 | Obstacle realtime progression - 2D');title('Leader: Agent X3 | Follower: Agent X4 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X4_RealTimeObstacle.avi');
   open(v);
   for i=1:m
