...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX1.mat;               % Load save A
load LeaderX1.mat;                            % Load save B
load ObstacleX1.mat;                          % Load save C
load TimeWindowX1.mat;                        % Load save D 
load IntersectionPolyshapeX1.mat              % Load save E
load DetectionWindowX1.mat;                   % Load save F
load IntersectionPolyshapeX1P.mat             % Load save E (Puddle)
load ObstacleX1P.mat;                         % Load save C (Puddle)
load DetectionWindowX1P.mat;                  % Load save F (Puddle)
load TotalResultsX1.mat;                      % Load save G
...
v=VideoWriter('X2_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX2.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_2 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X1) - Follower(X2) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX2A=figure('Name','Leader: Agent X1 | Follower: Agent X2');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X1 | Follower: Agent X2');
...
close(v);
savefig(FigureX2A,'FigureX2A.fig');
...
save LeaderX2.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX2.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX2P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX2.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX2.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX2P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX2.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX2P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX2.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX2B=figure('Name','Leader:  X1 | Follower: Agent X2 | Obstacle realtime progression - 2D');title('Leader: Agent X1 | Follower: Agent X2 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X2_RealTimeObstacle.avi');
   open(v);
   for i=1:m
