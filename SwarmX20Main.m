...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX19.mat;              % Load save A
load LeaderX19.mat;                           % Load save B
load ObstacleX19.mat;                         % Load save C
load TimeWindowX19.mat;                       % Load save D 
load IntersectionPolyshapeX19.mat             % Load save E
load DetectionWindowX19.mat;                  % Load save F
load IntersectionPolyshapeX19P.mat             % Load save E (Puddle)
load ObstacleX19P.mat;                        % Load save C (Puddle)
load DetectionWindowX19P.mat;                 % Load save F (Puddle)
load TotalResultsX19.mat;                     % Load save G
...
v=VideoWriter('X20_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX20.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_20 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X19) - Follower(X20) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX20A=figure('Name','Leader: Agent X19 | Follower: Agent X20');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X19 | Follower: Agent X20');
...
close(v);
savefig(FigureX20A,'FigureX20A.fig');
...
save LeaderX20.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX20.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX20P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX20.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX20.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX20P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX20P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX20.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX20B=figure('Name','Leader:  X19 | Follower: Agent X20 | Obstacle realtime progression - 2D');title('Leader: Agent X19 | Follower: Agent X20 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X20_RealTimeObstacle.avi');
   open(v);
   for i=1:m
