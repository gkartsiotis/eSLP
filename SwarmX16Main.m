...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX15.mat;              % Load save A
load LeaderX15.mat;                           % Load save B
load ObstacleX15.mat;                         % Load save C
load TimeWindowX15.mat;                       % Load save D 
load IntersectionPolyshapeX15.mat             % Load save E
load DetectionWindowX15.mat;                  % Load save F
load IntersectionPolyshapeX15P.mat             % Load save E (Puddle)
load ObstacleX15P.mat;                        % Load save C (Puddle)
load DetectionWindowX15P.mat;                 % Load save F (Puddle)
load TotalResultsX15.mat;                     % Load save G
...
v=VideoWriter('X16_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX16.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_16 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X15) - Follower(X16) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX16A=figure('Name','Leader: Agent X15 | Follower: Agent X16');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X15 | Follower: Agent X16');
...
close(v);
savefig(FigureX16A,'FigureX16A.fig');
...
save LeaderX16.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX16.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX16P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX16.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX16.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX16P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX16.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX16P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX16.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX16B=figure('Name','Leader:  X15 | Follower: Agent X16 | Obstacle realtime progression - 2D');title('Leader: Agent X15 | Follower: Agent X16 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X16_RealTimeObstacle.avi');
   open(v);
   for i=1:m
