...
load LeaderX0.mat;                            % Load save B
load ObstacleX0.mat;                          % Load save C
load TimeWindowX0.mat;                        % Load save D 
load IntersectionPolyshapeX0.mat              % Load save E
load DetectionWindowX0.mat;                   % Load save F
load IntersectionPolyshapeX0P.mat             % Load save E (Puddle)
load ObstacleX0P.mat;                         % Load save C (Puddle)
load DetectionWindowX0P.mat;                  % Load save F (Puddle)
load TotalResultsX0.mat;                      % Load save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Load data from leader
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Agent x_1 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
v=VideoWriter('X1_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX1.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_1 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X0) - Follower(X1) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX1A=figure('Name','Leader: Agent X0 | Follower: Agent X1');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X0 | Follower: Agent X1');
...
close(v);
savefig(FigureX1A,'FigureX1A.fig');
...
save LeaderX1.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX1.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX1P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX1.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX1.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX1P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX1.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX1P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX1.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX1B=figure('Name','Leader:  X0 | Follower: Agent X1 | Obstacle realtime progression - 2D');title('Leader: Agent X0 | Follower: Agent X1 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X1_RealTimeObstacle.avi');
   open(v);
   for i=1:m
