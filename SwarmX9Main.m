...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX8.mat;               % Load save A
load LeaderX8.mat;                            % Load save B
load ObstacleX8.mat;                          % Load save C
load TimeWindowX8.mat;                        % Load save D 
load IntersectionPolyshapeX8.mat              % Load save E
load DetectionWindowX8.mat;                   % Load save F
load IntersectionPolyshapeX8P.mat             % Load save E (Puddle)
load ObstacleX8P.mat;                         % Load save C (Puddle)
load DetectionWindowX8P.mat;                  % Load save F (Puddle)
load TotalResultsX8.mat;                      % Load save G
...
v=VideoWriter('X9_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX9.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_9 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X8) - Follower(X9) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX9A=figure('Name','Leader: Agent X8 | Follower: Agent X9');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X8 | Follower: Agent X9');
...
close(v);
savefig(FigureX9A,'FigureX9A.fig');
...
save LeaderX9.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX9.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX9P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX9.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX9.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX9P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX9.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX9P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX9.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX9B=figure('Name','Leader:  X8 | Follower: Agent X9 | Obstacle realtime progression - 2D');title('Leader: Agent X8 | Follower: Agent X9 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X9_RealTimeObstacle.avi');
   open(v);
   for i=1:m
