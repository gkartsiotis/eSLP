...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX18.mat;              % Load save A
load LeaderX18.mat;                           % Load save B
load ObstacleX18.mat;                         % Load save C
load TimeWindowX18.mat;                       % Load save D 
load IntersectionPolyshapeX18.mat             % Load save E
load DetectionWindowX18.mat;                  % Load save F
load IntersectionPolyshapeX18P.mat             % Load save E (Puddle)
load ObstacleX18P.mat;                        % Load save C (Puddle)
load DetectionWindowX18P.mat;                 % Load save F (Puddle)
load TotalResultsX18.mat;                     % Load save G
...
v=VideoWriter('X19_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX19.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_19 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X18) - Follower(X19) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX19A=figure('Name','Leader: Agent X18 | Follower: Agent X19');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X17 | Follower: Agent X18');
...
close(v);
savefig(FigureX19A,'FigureX19A.fig');
...
save LeaderX19.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX19.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX19P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX19.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX19.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX19P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX19.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX19P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX19.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX17B=figure('Name','Leader:  X18 | Follower: Agent X19 | Obstacle realtime progression - 2D');title('Leader: Agent X18 | Follower: Agent X19 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X19_RealTimeObstacle.avi');
   open(v);
   for i=1:m
