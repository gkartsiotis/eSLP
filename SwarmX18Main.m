...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX17.mat;              % Load save A
load LeaderX17.mat;                           % Load save B
load ObstacleX17.mat;                         % Load save C
load TimeWindowX17.mat;                       % Load save D 
load IntersectionPolyshapeX17.mat             % Load save E
load DetectionWindowX17.mat;                  % Load save F
load IntersectionPolyshapeX17P.mat             % Load save E (Puddle)
load ObstacleX17P.mat;                        % Load save C (Puddle)
load DetectionWindowX17P.mat;                 % Load save F (Puddle)
load TotalResultsX17.mat;                     % Load save G
...
save AlgorithmParametersX18.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_18 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X17) - Follower(X18) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX18A=figure('Name','Leader: Agent X17 | Follower: Agent X18');
...
close(v);
savefig(FigureX18A,'FigureX18A.fig');
...
save LeaderX18.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX18.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX18P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX18.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX18.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX18P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX18.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX18P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX18.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX17B=figure('Name','Leader:  X17 | Follower: Agent X18 | Obstacle realtime progression - 2D');title('Leader: Agent X17 | Follower: Agent X18 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X18_RealTimeObstacle.avi');
   open(v);
   for i=1:m
