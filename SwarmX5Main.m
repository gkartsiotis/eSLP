...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX4.mat;               % Load save A
load LeaderX4.mat;                            % Load save B
load ObstacleX4.mat;                          % Load save C
load TimeWindowX4.mat;                        % Load save D 
load IntersectionPolyshapeX4.mat              % Load save E
load DetectionWindowX4.mat;                   % Load save F
load IntersectionPolyshapeX4P.mat             % Load save E (Puddle)
load ObstacleX4P.mat;                         % Load save C (Puddle)
load DetectionWindowX4P.mat;                  % Load save F (Puddle)
load TotalResultsX4.mat;                      % Load save G
...
v=VideoWriter('X5_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX5.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_5 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X4) - Follower(X5) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX5A=figure('Name','Leader: Agent X4 | Follower: Agent X5');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X5 | Follower: Agent X4');
...
close(v);
savefig(FigureX5A,'FigureX5A.fig');
...
save LeaderX5.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX5.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX5P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX5.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX5.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX5P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX5.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX5P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX5.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX5B=figure('Name','Leader:  X4 | Follower: Agent X5 | Obstacle realtime progression - 2D');title('Leader: Agent X4 | Follower: Agent X5 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X5_RealTimeObstacle.avi');
   open(v);
   for i=1:m
