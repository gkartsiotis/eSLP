...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX13.mat;              % Load save A
load LeaderX13.mat;                           % Load save B
load ObstacleX13.mat;                         % Load save C
load TimeWindowX13.mat;                       % Load save D 
load IntersectionPolyshapeX13.mat             % Load save E
load DetectionWindowX13.mat;                  % Load save F
load IntersectionPolyshapeX13P.mat             % Load save E (Puddle)
load ObstacleX13P.mat;                        % Load save C (Puddle)
load DetectionWindowX13P.mat;                 % Load save F (Puddle)
load TotalResultsX13.mat;                     % Load save G
...
v=VideoWriter('X14_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX14.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_14 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X13) - Follower(X14) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX14A=figure('Name','Leader: Agent X13 | Follower: Agent X14');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X13 | Follower: Agent X14');
...
close(v);
savefig(FigureX14A,'FigureX14A.fig');
...
save LeaderX14.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX14.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX14P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX14.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX14.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX14P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX14.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX14P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX14.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX14B=figure('Name','Leader:  X13 | Follower: Agent X14 | Obstacle realtime progression - 2D');title('Leader: Agent X13 | Follower: Agent X14 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X14_RealTimeObstacle.avi');
   open(v);
   for i=1:m
