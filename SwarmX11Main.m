...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
load AlgorithmParametersX10.mat;              % Load save A
load LeaderX10.mat;                           % Load save B
load ObstacleX10.mat;                         % Load save C
load TimeWindowX10.mat;                       % Load save D 
load IntersectionPolyshapeX10.mat             % Load save E
load DetectionWindowX10.mat;                  % Load save F
load IntersectionPolyshapeX10P.mat             % Load save E (Puddle)
load ObstacleX10P.mat;                        % Load save C (Puddle)
load DetectionWindowX10P.mat;                 % Load save F (Puddle)
load TotalResultsX10.mat;                     % Load save G
...
v=VideoWriter('X11_Progression.avi');
v.FrameRate=1;
open(v);

save AlgorithmParametersX11.mat;        % Save setting so to be used by all x_k,k>=1 - Save A
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% End of Agent x_11 initialization
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------

% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
% Plot initial time window Leader(X9) - Follower(X110) 
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
FigureX11A=figure('Name','Leader: Agent X10 | Follower: Agent X11');
plot3(Leader(:,2),Leader(:,3),Leader(:,1),'Color','red');                                                                 
title('Leader: Agent X10 | Follower: Agent X11');
...
close(v);
savefig(FigureX11A,'FigureX11A.fig');
...
save LeaderX11.mat Leader;                                                  % Save leader's trajectory - Save B
...
save ObstacleX11.mat Obstacle;                    % Save obstacle positions and radius - Save C
...
save ObstacleX11P.mat Obstacle2;                    % Save obstacle positions and radius - Save C
clear m n i;
...
save TimeWindowX11.mat TW;                                % Save the new initial time window information - Save D
...
save IntersectionPolyshapeX11.mat LeaObsIntPoly; % Save E
clear m n i xmin xmax ymin ymax;
...
save IntersectionPolyshapeX11P.mat LeaObsIntPoly2; % Save E
clear m n i xmin xmax ymin ymax;
...
clear m n i xmin xmax ymin ymax PolyLowerTime PolyUpperTime PolyUnion XLimBou YLimBou P A C a b C X;
save DetectionWindowX11.mat LeaObsDetTW LeaObsDetCon;  % Save the time window detectuions and intersections realted information - Save F
...
clear m n i xmin2 xmax2 ymin2 ymax2 PolyLowerTime2 PolyUpperTime2 PolyUnion2 XLimBou2 YLimBou2 P2 A2 C2 a2 b2 C2 X2;
save DetectionWindowX11P.mat LeaObsDetTW2 LeaObsDetCon2;  % Save the time window detectuions and intersections realted information - Save F

% 4. Collect results
save TotalResultsX11.mat TotalResults;         % Save G
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if PlotRealTimeProgression==1
   FigureX11B=figure('Name','Leader:  X10 | Follower: Agent X11 | Obstacle realtime progression - 2D');title('Leader: Agent X10 | Follower: Agent X11 | Obstacle realtime progression - 2D');
   ...
   [m,n]=size(Obstacle);
   v=VideoWriter('X11_RealTimeObstacle.avi');
   open(v);
   for i=1:m
